...
- **Local & Private Lecture Upload** – Users can upload local video files and process private videos in addition to YouTube lectures.
- **Educational Content Classification** – Use a binary classifier to identify whether content is educational or non-educational. (Use LLM to classify content)
- **Agentic AI** – Introduce AI agents capable of autonomously deciding and executing learning-related tasks.
- **Tool Calling & LangGraph** – Implement tool calling and LangGraph for agent orchestration and multi-step workflows.
- **Playlist & Batch Ingestion** – Support ingestion and processing of multiple lectures/videos through a single batch job.
- **Multi-Language Support** – Support non-English lectures for transcription, processing, and learning-content generation.
- **User Accounts & Data Isolation** – Add authentication, authorization, and secure multi-user data isolation.
- **Revision Scheduling** – Introduce scheduled revision using spaced-repetition-based reminders for lectures, notes, and flashcards.
...
| Frontend | ReactJS | 19.2.x |
| Orchestration | LangChain | 0.3.x |
| Agent framework | LangGraph | 0.2.x |
| Vector DB | Qdrant (persistent, local) | 1.19.x |
...
| Transcription | faster-whisper tiny/small (local fallback) / youtube-transcript-api | 1.0.x |
...
| Object Storage | MinIO | latest version |
...
| FR-103 | Fetch captions via YouTube Transcript API first. (python library → youtube-transcript-api) | If captions exist, they are used; Whisper is not called. |
| FR-104 | Store audio file in local object storage (MinIO) | This allows the audio to be persisted and accessed later by the transcription and processing services. |
| FR-105 | Fallback to local faster-whisper when captions are absent. | Audio is downloaded with yt-dlp, converted to 16 kHz mono WAV with ffmpeg, and transcribed locally with faster-whisper tiny/small; a timestamped transcript is produced. |
| FR-106 | Clean the transcript. | Filler tokens in the set {um, uh, you know, like, so} at sentence start are removed; output has sentence casing and terminal punctuation. |
| FR-107 | Delete raw audio after transcription from Object Storage (MinIO). | The downloaded audio file is deleted instantly after job completion; only transcript text persists. |
...
flowchart LR
    ...
    D --> E["faster-whisper tiny/small (local)"]
...
| FR-203 | Embed chunks with BAAI/bge-base-en-v1.5 (sentence-transformers). | Every chunk has a 768-dimension vector stored in Qdrant. |
| FR-204 | One Qdrant collection per video, cosine distance. | Collection name = `video_id`; distance metric = cosine. |
...
| FR-303 | DSA template output structure. | We are not handling different video types separately; instead, we use the same template for all types of videos. |
| FR-304 | Generic template for non-DSA lectures. | We are not handling different video types separately; instead, we use the same template for all types of videos. |
...
## 7. Agent requirements (FR-6xx) - V2
...
### 7.1 Tool catalog (exact signatures) - V2
...
- **Storage backend:** Qdrant persistent client at path `./data/qdrant`; notes / transcripts as JSON at `./data/records`.
...
## 13. Milestones (tied to the action plan phases)
